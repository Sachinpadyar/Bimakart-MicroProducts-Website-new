...
export function BuyPolicyForm({ product, isLoading }: Props) {
  ...
  const renderField = (field: any) => {
      ...
      return (
        <div key={_id} className="w-full">
          <Select
            ...
            // value={...} // Manage state if needed, focusing on UI now
            onChange={() => { }}
    ...
    if (dataType === 'radio') {
      // Simple radio implementation or reuse logic
      return (
          ...
          <div className="flex flex-wrap gap-4">
            {options.map((opt: string) => (
              <div key={opt} className="flex items-center gap-2">
                <input type="radio" name={_id} id={`${_id}-${opt}`} className="w-4 h-4 text-orange-600 focus:ring-orange-500 border-gray-300" />
                <label htmlFor={`${_id}-${opt}`} className="text-sm text-gray-700 cursor-pointer">{opt}</label>
              </div>
            ))}
          </div>
        </div>
    ...
    if (dataType === 'checkbox') {
      return (
        <div key={_id} className="flex items-center gap-2 mt-6">
          <input type="checkbox" id={_id} className="w-4 h-4 text-orange-600 rounded border-gray-300 focus:ring-orange-500" />
          <label htmlFor={_id} className="text-sm font-medium text-gray-700 cursor-pointer">{label}</label>
        </div>
      );
    }

    // Default text/number/etc
    return <Input key={_id} label={label} placeholder="" />;
  };
